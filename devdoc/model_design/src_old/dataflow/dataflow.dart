...
class EventSource {
  void addEventListener<D>(String type, EventReceive<D> send) {}
}
...
class Dataflow<D> {
  ...
  Dataflow<D> events(
    ...
    final s = EventStream<D>(filter, apply);
    final _send = (Event<D> e) {
      e.dataflow = this;
      try {
        s.receive(e);
      } catch (err) {
  ...
  Dataflow<D> onStream(
    EventStream<D> source,
    Operator<dynamic, D> Function(Event<D>) target,
